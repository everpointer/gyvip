  ...
  if ($credit > 0) {
    ...
    if ($response) {
      ...
      if ($result != true) {
      	return array(
          "success" => false,
          "error" => "Fail to createMemberPrize when cardNumber:" . $cardNumber . " initially bind."
        );
      }
    } else if(!$response || isset($response->data->error)) {
      return array(
        "success" => false,
        "error" => "Fail to call kmtkDepositScore for cardNumber:" . $cardNumber . ", due to: " . $response->data->error
      );
    } else if (!empty($response->message)) {
      return array(
        "success" => false,
        "error" => "Fail to call kmtkDepositScore for cardNumber:" . $cardNumber . ", due to: " . $response->message
      );
    }
  } 
  return true;
